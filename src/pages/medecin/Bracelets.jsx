...
export default function Bracelets() {
  ...
  const qr = useMemo(() => qrDataUrl(payload(affiche)), [affiche])
  ...
  const imprimerBracelet = () => {
    imprimer({
      ...
      corps: `<div class="b"><div class="bar"></div><img class="qr" src="${qr}" alt=""><div class="t"><div class="h">HÔPITAL M&amp;M · ${esc(d.service).toUpperCase()}</div><div class="n">${esc(d.nom)}</div><div class="m">${esc(d.dossier)} · NÉ(E) LE ${esc(affiche.naissance)} · GR. ${esc(d.groupe)}</div></div>${allergie ? `<div class="a"><span>ALLERGIE</span><b>${esc(d.allergies)}</b></div>` : ''}</div>`,
    })
    ...
    <>
      ...
      <div className="cadre" style={{ display: 'grid', gap: 22 }}>
        <div className="grille-champs large">
          ...
          <ChampDate label="Date de naissance" valeur={d.naissance} onChange={maj('naissance')} max={valeurDate()} />
          ...
          <div style={{ overflowX: 'auto', paddingBottom: 4 }}>
            <div style={{ display: 'flex', alignItems: 'stretch', background: '#fff', border: '1px solid var(--filet-fort)', minWidth: 640, maxWidth: 860, height: 104 }}>
              ...
              <div style={{ flex: 1, minWidth: 0, padding: '10px 14px 10px 4px', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 4, lineHeight: 1.25 }}>
                ...
                <div className="mono" style={{ fontSize: 12, color: 'var(--anthracite)', whiteSpace: 'nowrap' }}>{d.dossier} · NÉ(E) LE {affiche.naissance} · GR. {d.groupe}</div>
              </div>
              {allergie && (
        ...
        <div className="rangee-btn">
          ...
          <button type="button" className="btn" onClick={() => nav('/medecin/scanner', { state: { test: payload(affiche) } })}>Tester dans le scanner</button>
        </div>
      </div>
    </>
  )
